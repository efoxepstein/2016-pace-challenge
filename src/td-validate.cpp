...
unsigned pure_stou(const std::string &s) {
  ...
  always_assert(!(result > std::numeric_limits<unsigned>::max()));
  return result;
}
...
struct tree_decomposition {
   ...
   * Get the u-th bag
   */
  vertex_t &get_bag(unsigned u) { return bags.at(u); }
   ...
   */
  bool tree_dfs(std::vector<int> &seen, unsigned root, unsigned parent,
                unsigned &num_seen) {
    if (seen[root] != 0) {
      return false;
    }
...
void read_solution(const std::vector<std::string> &tokens) {
  always_assert(!(current_state != COMMENT_SECTION));
  current_state = S_LINE;
  always_assert(!(tokens.size() != 5 || tokens[1] != "td"));

  n_decomp = pure_stou(tokens[2]);
  width = pure_stou(tokens[3]);
  n_graph = pure_stou(tokens[4]);
  always_assert(!(width > n_graph));
}
...
void read_bag(const std::vector<std::string> &tokens) {
  ...
  always_assert(!(current_state != BAGS));

  always_assert(!(tokens.size() < 2));

  unsigned bag_num = pure_stou(tokens[1]);
  always_assert(
      !(bag_num < 1 || bag_num > n_decomp || bags_seen[bag_num - 1] != 0));
 ...
 */
void read_decomp_edge(const std::vector<std::string> &tokens,
  ...
  always_assert(!(current_state != EDGES));

  unsigned s = pure_stou(tokens[0]);
  unsigned d = pure_stou(tokens[1]);
  always_assert(!(s < 1 || d < 1 || s > n_decomp || d > n_decomp));
  T.add_edge(s - 1, d - 1);
}
...
void read_problem(const std::vector<std::string> &tokens, graph &g) {
  ...
  always_assert(!(tokens.size() != 4 || tokens[1] != "tw"));
...
void read_graph_edge(const std::vector<std::string> &tokens, graph &g) {
  ...
  unsigned s = pure_stou(tokens[0]);
  unsigned d = pure_stou(tokens[1]);
  always_assert(!(s < 1 || d < 1 || s > n_vertices || d > n_vertices));
  g.add_edge(s - 1, d - 1);
}
...
void read_graph(std::ifstream &fin, graph &g) {
  ...
  always_assert(!(!fin.is_open()));

  std::string line;
  std::string delimiter = " ";

  while (std::getline(fin, line)) {
    always_assert(!(line == "" || line == "\n"));
  ...
  always_assert(!(g.num_edges != n_edges));
}
...
void read_tree_decomposition(std::ifstream &fin, tree_decomposition &T) {
  ...
  always_assert(!(width != real_width));
}
 ...
 */
bool check_vertex_coverage(tree_decomposition &T, graph &) {
  if (!(n_graph == n_vertices)) {
    std::cerr
        << "Error: .gr and .td disagree on how many vertices the graph has"
        << std::endl;
    return false;
...
bool check_edge_coverage(tree_decomposition &T, graph &g) {
  ...
  for (unsigned i = 0; i < T.bags.size() && g.num_edges > 0; i++) {
    ...
    for (std::set<unsigned>::iterator head = it_bag.begin();
         head != it_bag.end(); head++) {
      for (auto tail = g.neighbors(*head - 1).begin();
           tail != g.neighbors(*head - 1).end(); tail++) {
        if (it_bag.find(*tail + 1) != it_bag.end()) {
          to_remove.push_back(std::pair<unsigned, unsigned>(*head, *tail));
        }
      }
      for (std::vector<std::pair<unsigned, unsigned>>::iterator rem_it =
               to_remove.begin();
           rem_it != to_remove.end(); rem_it++) {
...
bool check_connectedness(tree_decomposition &T) {
  ...
  while (T.num_vertices > 0) {
    ...
    for (unsigned i = 0; i < T.bags.size(); i++) {
       ...
       */
      if (T.neighbors(i).size() == 1 ||
          (T.neighbors(i).size() == 0 && bags_removed.at(i) == 0)) {
        std::set<unsigned> intersection;
        std::set_intersection(
            forgotten.begin(), forgotten.end(), T.get_bag(i).begin(),
            T.get_bag(i).end(),
            std::inserter(intersection, intersection.begin()));
        if (!intersection.empty()) {
          printf("Found bad:\n");
          for (unsigned i : intersection)
            printf("  %u\n", i);
          return false;
        }
        ...
        if (T.neighbors(i).size() > 0) {
          ...
          std::set_difference(T.get_bag(i).begin(), T.get_bag(i).end(),
                              T.get_bag(parent).begin(),
                              T.get_bag(parent).end(),
...
bool is_valid_decomposition(tree_decomposition &T, graph &g) {
  if (!T.is_tree()) {
    ...
    std::cerr
        << "Error: some vertex induces disconnected components in the tree"
        << std::endl;
    return false;
  } else {
    return true;
...
int main(int argc, char **argv) {
  ...
  if (argc < 2 || argc > 3) {
    ...
    std::cerr << std::endl;
    std::cerr << "Validates syntactical and semantical correctness of .gr and "
                 ".td files. If only a .gr file is given, it validates the "
                 "syntactical correctness of that file." << std::endl;
    exit(1);
  }
  tree_decomposition T;
  graph g;
